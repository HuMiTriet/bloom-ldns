...
ldns_status ldns_key_new_frm_engine(ldns_key** key, ENGINE* e, char* key_id,
                                    ldns_algorithm alg)
{
  ldns_key* k;
...
static EVP_PKEY* ldns_key_new_frm_fp_gost_l(FILE* fp, int* line_nr)
{
  char token[16384];
  ...
  while (strlen(token) < 96) {
    ...
    if (ldns_fget_token_l(fp, token + strlen(token), "\n",
                          sizeof(token) - strlen(token), line_nr)
        == -1)
      return NULL;
  }
...
static int ldns_EC_KEY_calc_public(EC_KEY* ec)
{
  EC_POINT* pub_key;
  ...
  if (!EC_POINT_mul(group, pub_key, EC_KEY_get0_private_key(ec), NULL, NULL,
                    NULL)) {
    EC_POINT_free(pub_key);
    return 0;
  }
...
static EVP_PKEY* ldns_key_new_frm_fp_ecdsa_l(FILE* fp, ldns_algorithm alg,
                                             int* line_nr)
{
  char token[16384];
  ldns_rdf* b64rdf = NULL;
...
static EVP_PKEY* ldns_ed25519_priv_raw(uint8_t* pkey, int plen)
{
  const unsigned char* pp;
  uint8_t buf[256];
  int buflen = 0;
...
static EVP_PKEY* ldns_key_new_frm_fp_ed25519_l(FILE* fp, int* line_nr)
{
  char token[16384];
  ldns_rdf* b64rdf = NULL;
  EVP_PKEY* evp_key;
  ...
  evp_key = ldns_ed25519_priv_raw(ldns_rdf_data(b64rdf), (int)ldns_rdf_size(b64rdf));
  ldns_rdf_deep_free(b64rdf);
  return evp_key;
}
#endif
...
#ifdef USE_ED448
/** turn private key buffer into EC_KEY structure */
static EVP_PKEY* ldns_ed448_priv_raw(uint8_t* pkey, int plen)
{
  const unsigned char* pp;
  uint8_t buf[256];
  int buflen = 0;
  uint8_t pre[] = {0x30, 0x47, 0x02, 0x01, 0x00, 0x30, 0x05, 0x06,
...
static EVP_PKEY* ldns_key_new_frm_fp_ed448_l(FILE* fp, int* line_nr)
{
  char token[16384];
  ldns_rdf* b64rdf = NULL;
  EVP_PKEY* evp_key;
  ...
  evp_key = ldns_ed448_priv_raw(ldns_rdf_data(b64rdf), (int)ldns_rdf_size(b64rdf));
  ldns_rdf_deep_free(b64rdf);
  return evp_key;
}
#endif
...
ldns_status ldns_key_new_frm_fp_l(ldns_key** key, FILE* fp, int* line_nr)
{
  ...
  case LDNS_SIGN_ECC_GOST:
    ...
    ldns_key_set_evp_key(k, ldns_key_new_frm_fp_gost_l(fp, line_nr));
#ifndef S_SPLINT_S
    if (!k->_key.key) {
      ldns_key_free(k);
      return LDNS_STATUS_ERR;
  ...
  case LDNS_SIGN_ECDSAP384SHA384:
    ...
    ldns_key_set_evp_key(
      k, ldns_key_new_frm_fp_ecdsa_l(fp, (ldns_algorithm)alg, line_nr));
#ifndef S_SPLINT_S
    if (!k->_key.key) {
      ldns_key_free(k);
      return LDNS_STATUS_ERR;
  ...
  case LDNS_SIGN_ED25519:
    ldns_key_set_algorithm(k, alg);
    ldns_key_set_evp_key(k, ldns_key_new_frm_fp_ed25519_l(fp, line_nr));
#ifndef S_SPLINT_S
    if (!k->_key.key) {
      ldns_key_free(k);
      return LDNS_STATUS_ERR;
  ...
  case LDNS_SIGN_ED448:
    ldns_key_set_algorithm(k, alg);
    ldns_key_set_evp_key(k, ldns_key_new_frm_fp_ed448_l(fp, line_nr));
#ifndef S_SPLINT_S
    if (!k->_key.key) {
      ldns_key_free(k);
      return LDNS_STATUS_ERR;
...
RSA* ldns_key_new_frm_fp_rsa_l(FILE* f, int* line_nr)
{
  ...
  i = ldns_b64_pton((const char*)b, buf,
...
DSA* ldns_key_new_frm_fp_dsa_l(FILE* f, ATTR_UNUSED(int* line_nr))
{
  int i;
  char* d;
  DSA* dsa;
  ...
  i = ldns_b64_pton((const char*)d, buf,
...
unsigned char* ldns_key_new_frm_fp_hmac_l(FILE* f, ATTR_UNUSED(int* line_nr),
                                          size_t* hmac_size)
{
  size_t bufsz;
...
#ifdef USE_GOST
static EVP_PKEY* ldns_gen_gost_key(void)
{
  EVP_PKEY_CTX* ctx;
  EVP_PKEY* p = NULL;
...
void ldns_key_set_flags(ldns_key* k, uint16_t f) { k->_extra.dnssec.flags = f; }

#ifdef HAVE_SSL
#ifndef S_SPLINT_S
void ldns_key_set_evp_key(ldns_key* k, EVP_PKEY* e) { k->_key.key = e; }
...
ldns_signing_algorithm ldns_key_algorithm(const ldns_key* k) { return k->_alg; }
...
#ifdef HAVE_SSL
#ifndef S_SPLINT_S
EVP_PKEY* ldns_key_evp_key(const ldns_key* k) { return k->_key.key; }
...
void* ldns_key_external_key(const ldns_key* k) { return k->_key.external_key; }
...
uint16_t ldns_key_flags(const ldns_key* k) { return k->_extra.dnssec.flags; }
...
uint16_t ldns_key_keytag(const ldns_key* k) { return k->_extra.dnssec.keytag; }

ldns_rdf* ldns_key_pubkey_owner(const ldns_key* k) { return k->_pubkey_owner; }
...
bool ldns_key_list_push_key(ldns_key_list* key_list, ldns_key* key)
{
  ...
  /* grow the array */
  keys = LDNS_XREALLOC(key_list->_keys, ldns_key*, key_count + 1);
  if (!keys) {
    return false;
  }
...
static bool ldns_key_rsa2bin(unsigned char* data, RSA* k, uint16_t* size)
{
  int i, j;
...
#ifdef USE_DSA
/* data pointer must be large enough (LDNS_MAX_KEYLEN) */
static bool ldns_key_dsa2bin(unsigned char* data, DSA* k, uint16_t* size)
{
  uint8_t T;
  const BIGNUM *p, *q, *g;
...
#ifdef USE_GOST
static bool ldns_key_gost2bin(unsigned char* data, EVP_PKEY* k,
                              uint16_t* size)
{
  int i;
...
#ifdef USE_ED25519
static bool ldns_key_ed255192bin(unsigned char* data, EVP_PKEY* k,
                                 uint16_t* size)
{
  int i;
  unsigned char* pp = NULL;
...
#ifdef USE_ED448
static bool ldns_key_ed4482bin(unsigned char* data, EVP_PKEY* k,
                               uint16_t* size)
{
  int i;
  unsigned char* pp = NULL;
...
ldns_rr* ldns_key2rr(const ldns_key* k)
{
  ...
  /* zero-th rdf - flags */
  ldns_rr_push_rdf(
    pubkey, ldns_native2rdf_int16(LDNS_RDF_TYPE_INT16, ldns_key_flags(k)));
  /* first - proto */
  ldns_rr_push_rdf(
    pubkey, ldns_native2rdf_int8(LDNS_RDF_TYPE_INT8, LDNS_DNSSEC_KEYPROTO));
  ...
  case LDNS_SIGN_RSASHA512:
    ldns_rr_push_rdf(
      pubkey, ldns_native2rdf_int8(LDNS_RDF_TYPE_ALG, ldns_key_algorithm(k)));
#ifdef HAVE_SSL
...
#ifdef USE_DSA
  case LDNS_SIGN_DSA:
    ldns_rr_push_rdf(pubkey, ldns_native2rdf_int8(LDNS_RDF_TYPE_ALG, LDNS_DSA));
#ifdef HAVE_SSL
  ...
  case LDNS_SIGN_ML_DSA_44:
    ...
    oqs_key* pub_oqs_key = ldns_key_external_key(k);
    if (pub_oqs_key == NULL) {
      ldns_rr_free(pubkey);
      return NULL;
    }
...
void ldns_key_free(ldns_key* key) { LDNS_FREE(key); }
...
char* ldns_key_get_file_base_name(const ldns_key* key)
{
  ...
  ldns_buffer_printf(buffer, "+%03u+%05u", ldns_key_algorithm(key),
                     ldns_key_keytag(key));
...
ldns_signing_algorithm ldns_get_signing_algorithm_by_name(const char* name)
{
  ...
  ldns_lookup_table aliases[] = {/* from bind dnssec-keygen */
