...
{
  return fmt && (fmt->flags & LDNS_FMT_RFC3597) && ((ldns_output_format_storage*)fmt)->bitmap && ldns_nsec_bitmap_covers_type(((ldns_output_format_storage*)fmt)->bitmap, t);
}
                             ...
                             const ldns_output_format* fmt, const ldns_rdf* rdf)
{
  ...
  if (!ldns_output_format_covers_type(fmt, data) && ldns_rr_descript(data) && ldns_rr_descript(data)->_name) {
                             ...
                             const ldns_output_format* fmt, const ldns_rdf* rdf)
{
  ...
  while ((size_t)(pos + 2) < ldns_rdf_size(rdf)) {
    ...
    for (bit_pos = 0; bit_pos < (bitmap_length) * 8; bit_pos++) {
      ...
      if (!ldns_output_format_covers_type(fmt, type) && ldns_rr_descript(type) && ldns_rr_descript(type)->_name) {
...
ldns_rdf2buffer_str_int16_data(ldns_buffer* output, const ldns_rdf* rdf)
{
  ...
  if (ldns_rdf_size(rdf) > 2) {
    ...
    if (ldns_rdf_size(rdf) > 2 && ldns_b64_ntop(ldns_rdf_data(rdf) + 2, ldns_rdf_size(rdf) - 2, b64, size)) {
      ldns_buffer_printf(output, "%s", b64);
    }
    LDNS_FREE(b64);
  }
...
ldns_rdf2buffer_str_unquoted(ldns_buffer* output, const ldns_rdf* rdf)
{
  ...
  for (i = 0; i < amount; i++) {
    ...
    if (isprint((int)ch) || ch == '\t') {
      if (ch == '\"' || ch == '\\' || ch == '\'' || ch == '(' || ch == ')' || isspace((int)ch))
        ldns_buffer_printf(output, "\\%c", ch);
      else
        ldns_buffer_printf(output, "%c", ch);
    }
...
ldns_rdf2buffer_str_hip(ldns_buffer* output, const ldns_rdf* rdf)
{
  ...
  if ((hit_size = data[0]) == 0 || (pk_size = ldns_read_uint16(data + 2)) == 0 || rdf_size < (size_t)hit_size + pk_size + 4) {
  ...
  if (ldns_buffer_reserve(output,
    ...
    if (written > 0 && written < (int)ldns_buffer_remaining(output)) {
...
ldns_rdf2buffer_str_amtrelay(ldns_buffer* output, const ldns_rdf* rdf)
{
  ...
  case 2:
    ...
    offset += LDNS_IP6ADDRLEN;
    relay = ldns_rdf_new(LDNS_RDF_TYPE_AAAA,
...
svcparam_ech2buffer_str(ldns_buffer* output, size_t sz, uint8_t* data)
{
  ...
  written = ldns_b64_ntop(data, sz, (char*)ldns_buffer_current(output), str_sz);
  if (written > 0)
                       ...
                       const ldns_output_format* fmt, const ldns_rr* rr)
{
  ...
  for (i = 0; i < ldns_rr_rd_count(rr); i++) {
    ...
    if ((fmt_st->flags & LDNS_FMT_ZEROIZE_RRSIGS) && (ldns_rr_get_type(rr) == LDNS_RR_TYPE_RRSIG) && ((/* inception  */ i == 4 && ldns_rdf_get_type(ldns_rr_rdf(rr, 4)) == LDNS_RDF_TYPE_TIME) || (/* expiration */ i == 5 && ldns_rdf_get_type(ldns_rr_rdf(rr, 5)) == LDNS_RDF_TYPE_TIME) || (/* signature  */ i == 8 && ldns_rdf_get_type(ldns_rr_rdf(rr, 8)) == LDNS_RDF_TYPE_B64))) {

      ldns_buffer_printf(output, "(null)");
      status = ldns_buffer_status(output);
    }
    else if ((fmt_st->flags & LDNS_FMT_PAD_SOA_SERIAL) && (ldns_rr_get_type(rr) == LDNS_RR_TYPE_SOA) &&
             /* serial */ i == 2 && ldns_rdf_get_type(ldns_rr_rdf(rr, 2)) == LDNS_RDF_TYPE_INT32) {
  ...
  if (ldns_rr_rd_count(rr) > 0) {
    ...
    case LDNS_RR_TYPE_DNSKEY:
      ...
      if ((fmt_st->flags & LDNS_COMMENT_KEY_TYPE) && (flags & LDNS_KEY_ZONE_KEY)) {
    ...
    case LDNS_RR_TYPE_DS:
      if ((fmt_st->flags & LDNS_COMMENT_BUBBLEBABBLE) && ldns_rr_rdf(rr, 3) != NULL) {
    ...
    case LDNS_RR_TYPE_NSEC3:
      if (!(fmt_st->flags & LDNS_COMMENT_FLAGS) && !(fmt_st->flags & LDNS_COMMENT_NSEC3_CHAIN)) {
        break;
      }
      ldns_buffer_printf(output, " ;{");
      if ((fmt_st->flags & LDNS_COMMENT_FLAGS)) {
        ...
        if (fmt_st->flags & LDNS_COMMENT_NSEC3_CHAIN && fmt_st->hashmap != NULL) {
          ldns_buffer_printf(output, ", ");
        }
      }
      if (fmt_st->flags & LDNS_COMMENT_NSEC3_CHAIN && fmt_st->hashmap != NULL) {
