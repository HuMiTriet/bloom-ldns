...
find_key_in_zone(ldns_rr* pubkey_gen, ldns_zone* zone)
{
  ...
  for (key_i = 0;
    ...
    if (ldns_rr_get_type(pubkey) == LDNS_RR_TYPE_DNSKEY && (ldns_calc_keytag(pubkey) == ldns_calc_keytag(pubkey_gen) ||
...
int str2zonemd_signflag(const char* str, const char** reason)
{
  ...
  static const char* reasons[] = {
    "Unknown <scheme>, should be \"simple\"", "Syntax error in <hash>, should be \"sha384\" or \"sha512\"", "Unknown <hash>, should be \"sha384\" or \"sha512\""};
...
int main(int argc, char* argv[])
{
  ...
  while ((c = getopt(argc, argv, "a:bde:f:i:k:no:ps:t:uvz:ZAUE:K:")) != -1) {
    ...
    case 'a':
      ...
      break;
    case 'b':
      ldns_output_format_set(fmt, LDNS_COMMENT_FLAGS | LDNS_COMMENT_LAYOUT | LDNS_COMMENT_NSEC3_CHAIN | LDNS_COMMENT_BUBBLEBABBLE);
      break;
    case 'd':
      add_keys = false;
      break;
    case 'e':
      ...
      if (strlen(optarg) == 8 && sscanf(optarg, "%4d%2d%2d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday)) {
        tm.tm_year -= 1900;
        tm.tm_mon--;
        check_tm(tm);
        expiration = (uint32_t)ldns_mktime_from_utc(&tm);
      }
      else if (strlen(optarg) == 14 && sscanf(optarg, "%4d%2d%2d%2d%2d%2d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour, &tm.tm_min, &tm.tm_sec)) {
        tm.tm_year -= 1900;
        tm.tm_mon--;
        check_tm(tm);
        expiration = (uint32_t)ldns_mktime_from_utc(&tm);
    ...
    case 'i':
      ...
      if (strlen(optarg) == 8 && sscanf(optarg, "%4d%2d%2d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday)) {
        ...
        tm.tm_mon--;
        check_tm(tm);
        inception = (uint32_t)ldns_mktime_from_utc(&tm);
      }
      else if (strlen(optarg) == 14 && sscanf(optarg, "%4d%2d%2d%2d%2d%2d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour, &tm.tm_min, &tm.tm_sec)) {
        tm.tm_year -= 1900;
        tm.tm_mon--;
    ...
    case 's':
      ...
      for (c = 0; c < (int)strlen(optarg); c += 2) {
        if (isxdigit((int)optarg[c]) && isxdigit((int)optarg[c + 1])) {
          nsec3_salt[c / 2] = (uint8_t)ldns_hexdigit_to_int(optarg[c]) * 16 + ldns_hexdigit_to_int(optarg[c + 1]);
        }
        else {
  ...
  if (ldns_dnssec_zone_add_rr(signed_zone, ldns_zone_soa(orig_zone)) != LDNS_STATUS_OK) {
    fprintf(stderr,
